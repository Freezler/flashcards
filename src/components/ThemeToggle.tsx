...
function ThemeToggle({ showAccentToggle = true }: ThemeToggleProps): React.JSX.Element {
  ...
  const toggleThemeMode = (): void => {
    const newMode: ThemeMode =
      themeState.mode === 'light' ? 'dark' : themeState.mode === 'dark' ? 'auto' : 'light'
  ...
  return (
    <div className="theme-controls glass-card">
      <button
        ...
        title={`Current: ${getThemeLabel()}. Click to cycle.`}
      >
        <span className="theme-toggle__icon magical-glow">{getThemeIcon()}</span>
        <span className="theme-toggle__label">{getThemeLabel()}</span>
      </button>
      ...
      >
        <span className="accent-toggle__icon magical-glow">{getAccentIcon()}</span>
