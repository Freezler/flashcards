...
export const validationRules = {
  ...
  minLength: (fieldName: string, minLength: number) => (value: string) =>
    value?.length < minLength ? `${fieldName} moet minimaal ${minLength} karakters bevatten` : undefined,
  
  maxLength: (fieldName: string, maxLength: number) => (value: string) =>
    value?.length > maxLength ? `${fieldName} mag maximaal ${maxLength} karakters bevatten` : undefined,
}